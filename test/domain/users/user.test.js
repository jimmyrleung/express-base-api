...
describe('User class', () => {
  describe('#Validation', () => {
    ...
    test('#should be invalid when name is missing', async (done) => {
      ...
      const isValid = await user.isValid();
      const includesErrorMessage = user.errors
        .includes(userConstants.USER_NAME_REQUIRED_MESSAGE);
    ...
    test('#should be invalid when email is missing', async (done) => {
      ...
      const isValid = await user.isValid();
      const includesErrorMessage = user.errors
        .includes(userConstants.USER_EMAIL_REQUIRED_MESSAGE);
    ...
    test('#should be invalid when password is missing and the flag \'validatePassword\' is active', async (done) => {
      ...
      const isValid = await user.isValid();
      const includesErrorMessage = user.errors
        .includes(userConstants.USER_PASSWORD_REQUIRED_MESSAGE);
    ...
    test('#should be invalid if the user\'s name length is less than the configured min length', async (done) => {
      ...
      for (let i = 1; i < userConstants.USER_NAME_MIN_LENGTH; i++) {
      ...
      const isValid = await user.isValid();
      const includesErrorMessage = user.errors
        .includes(userConstants.USER_NAME_MIN_LENGTH_MESSAGE);
    ...
    test('#should be invalid if the user\'s name length is greater than the configured max length', async (done) => {
      ...
      for (let i = 0; i <= userConstants.USER_NAME_MAX_LENGTH; i++) {
      ...
      const isValid = await user.isValid();
      const includesErrorMessage = user.errors
        .includes(userConstants.USER_NAME_MAX_LENGTH_MESSAGE);
    ...
    test('#should be invalid if the user\'s email isn\'t in a valid format.', async (done) => {
      ...
      const userIncludesErrorMessage = user.errors
        .includes(userConstants.USER_EMAIL_FORMAT_MESSAGE);

      const user2IncludesErrorMessage = user2.errors
        .includes(userConstants.USER_EMAIL_FORMAT_MESSAGE);

      expect(userIsValid).toBeFalsy();
      expect(user2IsValid).toBeFalsy();
      expect(userIncludesErrorMessage).toBeTruthy();
      expect(user2IncludesErrorMessage).toBeTruthy();
      done();
    });
  });
});
